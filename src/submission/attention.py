...
def precompute_rotary_emb(dim, max_positions):
    ...
    half_dim = dim // 2
    # theta_i = 1/10000^(2(i-1)/dim) for i in [1, dim/2]
    # 0-indexed: exponents are 0, 2/dim, 4/dim, ..., (dim-2)/dim
    theta = 1.0 / (10000 ** (torch.arange(0, half_dim).float() * 2.0 / dim))
    # positions t = 0, 1, ..., max_positions-1
    positions = torch.arange(0, max_positions).float()
    # outer product: (max_positions, half_dim)
    freqs = torch.outer(positions, theta)
    # stack [cos, sin] -> (max_positions, half_dim, 2)
...
def apply_rotary_emb(x, rope_cache):
    ...
    # Reshape pairs of dims as complex numbers: (B, n_head, T, head_size//2, 2)
    x_r = x.float().reshape(B, n_head, T, head_size // 2, 2)
    # View as complex: (B, n_head, T, head_size//2)
    x_complex = torch.view_as_complex(x_r.contiguous())

    # Truncate rope_cache to current sequence length: (T, head_size//2, 2)
    rope = rope_cache[:T].to(x.device)
    # View as complex: (T, head_size//2)
    rope_complex = torch.view_as_complex(rope.float().contiguous())

    # Apply rotation (broadcast over batch and head dims)
    rotated = x_complex * rope_complex.unsqueeze(0).unsqueeze(0)

    # Back to real: (B, n_head, T, head_size//2, 2) -> (B, n_head, T, head_size)
    rotated_x = torch.view_as_real(rotated).reshape(B, n_head, T, head_size).type_as(x)
    ### END CODE HERE
    return rotated_x
