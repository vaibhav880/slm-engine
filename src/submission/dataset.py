...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Step 1: randomly truncate document to length in [4, block_size*3/4]
        truncated_len = random.randint(4, int(self.block_size * 3 / 4))
        truncated_doc = doc[:truncated_len]

        # Step 2: choose masked_content length (~1/4 of truncated_len on average)
        masked_content_len = random.randint(1, max(1, int(truncated_len / 2)))
        masked_content_start = random.randint(0, truncated_len - masked_content_len)

        prefix = truncated_doc[:masked_content_start]
        masked_content = truncated_doc[masked_content_start:masked_content_start + masked_content_len]
        suffix = truncated_doc[masked_content_start + masked_content_len:]

        # Step 3: rearrange as: prefix MASK suffix MASK masked_content MASK pads
        ...
        # Step 4: x = masked_string[:-1], y = masked_string[1:]
        x = masked_string[:-1]
        y = masked_string[1:]
